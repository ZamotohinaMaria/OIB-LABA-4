...
def check_hash(x: int) -> tuple:
    ...
    Args:
        x (int): принимает номер карты

    Returns:
        tuple: в случае соответствия хеша возвращает БИН и номер карты
    """
    x = str(x).zfill(6)
    for b in BIN:
        if hashlib.sha256(
                f'{b}{x}{data["last_num"]}'.encode()).hexdigest() == data["hash"]:
            return (b, x)
    return False
...
def find_card_num() -> str:
    ...
    with mp.Pool(processes=5) as p:
        for result in p.map(check_hash, range(0, 1000000)):
            if result:
                logging.info(
                    f'Number of card: {result[0]}-{result[1]}-{data["last_num"]}')
                p.terminate()
    ...
    with mp.Pool(processes=c) as p:
        for result in p.map(check_hash, range(0, 1000000)):
            if result:
                ...
                logging.info(
                    f'Number of card: {result[0]}{result[1]}{data["last_num"]}, cores = {c}')
                p.terminate()
                return end - start
